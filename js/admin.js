(function () {
  ...
  async function handleLogin(e) {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/admin/owner-login`, {
        method: "POST",
          ...
          <form id="medicine-form">
            ...
            <label style="display: block; margin-bottom: 16px;">
              ...
              <select name="category" required style="width: 100%; padding: 8px 12px; border: 1px solid var(--border); border-radius: 4px; margin-top: 4px;">
                ...
                <option value="Syrup" ${
                  medicine?.category === "Syrup" ? "selected" : ""
                }>Syrup</option>
                <option value="Injection" ${
                  medicine?.category === "Injection" ? "selected" : ""
                }>Injection</option>
                <option value="Solution" ${
                  medicine?.category === "Solution" ? "selected" : ""
                }>Solution</option>
              </select>
            </label>
            ...
            <label style="display: block; margin-bottom: 16px;">
              Stock Quantity *
              <input type="number" name="stockCount" value="${
                medicine?.stockCount || ""
              }" min="0" required 
                     style="width: 100%; padding: 8px 12px; border: 1px solid var(--border); border-radius: 4px; margin-top: 4px;" />
            </label>
          ...
          <form id="user-form">
            ...
            <label style="display: block; margin-bottom: 16px;">
              ...
              <select name="role" required style="width: 100%; padding: 8px 12px; border: 1px solid var(--border); border-radius: 4px; margin-top: 4px;">
                ...
                }>Staff (Orders only)</option>
                <option value="MANAGER" ${
                  user?.role === "MANAGER" ? "selected" : ""
                }>Manager (Orders + Inventory)</option>
                <option value="ADMIN" ${
  ...
  async function toggleUserStatus(userId) {
    try {
      const token = sessionStorage.getItem("adminToken");
      const response = await fetch(
        `${API_BASE}/api/admin/users/${userId}/toggle`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
